...
class floSocket
{
    ...
    public $event_handler = array();
    public $echoback = false;
    public $listening = false;
    public $failsafe_timeout = -1;
    public $tick_length;

    // Class initialization.
    public function __construct($address = false, $port = "9999", $tick = 250000)
    {
...
//                  echo "Channel attribue : ".$channel."\n";
    ...
    public function stop($disconnect_message = "Server shutdown.", $error = false)
    {
    ...
    // Set event handlers
    public function set_handler($handler_id, $handler_function)
    {
        $this->event_handler[$handler_id] = $handler_function;
    }

    // Send data to active channel.
    public function write($channel, $text)
    {
    ...
    // Close channel.
    public function close($channel, $disconnect_message = "", $error = false)
    {
    ...
    public function getFreeChannel()
    {

        if (count($this->sockets) > 0) {
            $NewSocket = array_pop(array_keys($this->sockets)) + 1;
        } else {
            $NewSocket = 0;
        }
    ...
    public function close_all($disconnect_message, $error = false)
    {
    ...
    public function get_remote_address($channel)
    {
        if (isset($this->sockets[$channel])) {
    ...
    public function get_remote_port($channel)
    {
        if (isset($this->sockets[$channel])) {
            socket_getpeername($this->sockets[$channel], $remoteaddress, $remoteport);
            return $remoteport;
    ...
    public function get_socket_resource($channel)
    {
        return $this->sockets[$channel];
    }

    // Set echoback.
    public function set_echo($value = true)
    {
        $this->echoback = $value;
    }
...
class clsDaemonize
{
    ...
    public $logFP; // Log file File Pointer
    public $sdProcess = ""; // Name of the Shutdown Process Call Back
     ...
     */
    public function __constructor($log_file = "")
    {
     ...
     */
    public function setShutdownProc($sdProc)
    {
     ...
     */
    public function fork($child_proc)
    {
     ...
     */
    public function setup_sighandler($signal, $function)
    {
